...
def get_cifar10(batch_size=128, num_workers=0, n_train=None, n_test=None, seed=42):
    """Load CIFAR-10 dataset with optional subsampling and stratification."""
    transform = transforms.Compose([
        transforms.ToTensor(),
        transforms.Normalize((0.4914, 0.4822, 0.4465), (0.2470, 0.2435, 0.2616))
    ])

    train_data = datasets.CIFAR10(root='./data_cifar', train=True, download=True, transform=transform)
    test_data = datasets.CIFAR10(root='./data_cifar', train=False, download=True, transform=transform)
    ...
    print(f"Loaded CIFAR-10 with {len(train_data)} training samples and {len(test_data)} test samples.")
...
def build_model(architecture, activation, config, in_channels=1, input_size=(28, 28), num_classes=10, use_batchnorm=False, extra_convs=0):
    """
    Build a model based on architecture and activation type.
    
    Args:
        architecture: 'KKAN_Small' or 'KANC_MLP_Medium'
        activation: 'spline', 'rbf', or 'wavelet'
        config: Configuration dict for the activation
        in_channels: number of input channels (1 for MNIST, 3 for CIFAR)
        input_size: tuple (H, W) of input spatial dimensions
